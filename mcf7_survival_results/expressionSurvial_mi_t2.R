...
#setwd("brca_Survival")
pdf("brca_diffGenesW9-12vsW5-8_kmCurve_mi_intersect_0712.pdf")
pval_vec = vector()
...
for (i in rownames(cox.brca.sig)){
  ...
  data.quantile=quantile(data$gene)
  if (data.median != 0) {
    data1=data[which(data$gene>data.quantile[4]),]
    data1$gene=1
    data2=data[which(data$gene<=data.quantile[2]),]
    data2$gene=-1
    ...
    if (pval <= 0.05) {
      plot(mfit.byexpr,main= paste("DNB in BRCA","pVal=",pval),xlab="Time (Days)", ylab="Overall Survival Proportion",col=c("blue","red"))
